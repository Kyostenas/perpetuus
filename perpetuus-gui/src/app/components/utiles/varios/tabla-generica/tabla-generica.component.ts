...
import { ChangeDetectorRef, Component, computed, effect, EventEmitter, HostListener, Input, OnDestroy, OnInit, Output, Pipe, Signal, signal, TemplateRef, WritableSignal } from '@angular/core';
...
export class TablaGenericaComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    effect(() => {
      const QUERY_ACTUAL = this.control_queries.query_actual()
      if (QUERY_ACTUAL.paginacion) {
        this.ordenes_columnas.update(() => QUERY_ACTUAL.paginacion.campos_ordenamiento)
      }
      // console.log(this.control_queries.query_actual())
    })
  } 
  ...
  ngOnInit(): void {
    if (!this.control_queries.query_actual().paginacion) {
      ...
      this.control_queries.queries.paginacion.accion.definir(QUERY_DEFECTO)
    } else {
      this.emisor_paginacion.emit(this.control_queries.query_actual().paginacion)
    }
  }
  ...
  @Output('click_fila')
  emisor_click_fila: EventEmitter<OPCIONES_FILA_TABLA_GENERICA> = new EventEmitter()

  // (o-----------------------------------------( SOLO VARIABLES ))
   
  modo: WritableSignal<'tabla' | 'columnas'> = this.viewport.modo_tabla_generica
  modo_viewport: WritableSignal<'movil' | 'escritorio'> = this.viewport.modo_viewport
  ordenes_columnas: WritableSignal<Paginacion['campos_ordenamiento']> = signal({})
  ...
  si_no_existe(datos: any, valor_buscar: string, reemplazo_por_si_no: any) {
    if (!(datos[valor_buscar] !== undefined && datos[valor_buscar] !== null)) {
